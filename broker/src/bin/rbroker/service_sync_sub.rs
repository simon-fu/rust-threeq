...
// use async_raft::{AppData, AppDataResponse, Raft, RaftNetwork, RaftStorage, raft::{AppendEntriesRequest, AppendEntriesResponse, ClientWriteRequest, Entry, EntryPayload, InstallSnapshotRequest, InstallSnapshotResponse, MembershipConfig, VoteRequest, VoteResponse}, storage::{CurrentSnapshotData, HardState, InitialState}};
// use prost::Message;
...
// use std::{collections::{BTreeMap, HashMap, HashSet}, io::Cursor, sync::Arc, time::Duration};
...
//         let req = AppendRequest {data: bincode::serialize(&rpc).unwrap()};
...
//     async fn append_entries(&self, _target: u64, _rpc: AppendEntriesRequest<ClientRequest>) -> Result<AppendEntriesResponse> {
//         bail!("DummyRouter: Not implmented append_entries");
//     }

//     async fn install_snapshot(&self, _target: u64, _rpc: InstallSnapshotRequest) -> Result<InstallSnapshotResponse> {
...
//                 let rpc: AppendEntriesRequest<ClientRequest> = bincode::deserialize(&mut req.data).unwrap();
