...
impl PacketDecoder for PubRel {
    fn decode<B: Buf>(
        ...
        let puback = PubRel {
            pkid,
            reason: reason(ack_reason)?,
            properties: PubRelProperties::extract(bytes)?,
        };
...
impl PubRelProperties {
    ...
    pub fn extract<B: Buf>(mut bytes: &mut B) -> Result<Option<PubRelProperties>, Error> {
...
mod test {
    ...
    fn pubrel_parsing_works() {
        ...
        let mut pubrel_bytes = stream.split_to(fixed_header.frame_length()).freeze();
        let pubrel = PubRel::decode(Protocol::V5, &fixed_header, &mut pubrel_bytes).unwrap();
    ...
    fn pubrel_encoding_works() {
        ...
        pubrel.encode(Protocol::V5, &mut buf).unwrap();
